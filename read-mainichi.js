...
const getItems = async items => {
  ...
  for (let i = 0; i < items.length; i++) {
    ...
    const title = getMeta("title", itemArticleDoc);
    ...
    for (let video of videos) {
      video.outerHTML =
        "<strong>A video was embedded in the article. Open in a browser to view.</strong>";
    }
...
async function readMainichi() {
  const { data: frontPageData } = await axios.get(
  ...
  const subURLNodes = frontpage.querySelectorAll(".sub-box a");

  for (let url of mainURLNodes) {
    if (url.href.includes('article')) {
      allURLs.push(`https:` + url.href);
    }
  }
  
  for (let url of subURLNodes) {
    if (url.href.includes('article')) {
      allURLs.push(`https:` + url.href);
    }
  }
